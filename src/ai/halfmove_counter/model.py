...
from paths import HALFMOVE_COUNTER_MODEL
...
# Output the number of halfmoves
def Halfmove_Counter_Model():
...
def train_model(model=None, data_size=None, batch_size=128, epochs=1):
  def prep_data(fen):
    ...
    halfmoves = get_part(fen, Part.HALFMOVE_CLOCK)
    return encode_labels(board), float(halfmoves)

  train, valid = get_generators(INPUT_SHAPE, OUTPUT_SHAPE, data_size, batch_size, prep_data)

  if model is None:
    model = Halfmove_Counter_Model()
  ...
  model.save(HALFMOVE_COUNTER_MODEL)
